...
export function createTaskRoutes(): Hono<RouteEnv> {
  ...
  app.post("/", async (c) => {
    ...
    const sender = await db.getAgent(auth.agentAddress);
    if (!sender) {
      return c.json({ error: "Sender agent not registered" }, 403);
    }

    const recipient = await db.getAgent(body.to_agent);
    if (!recipient) {
    ...
    await db.touchAgent(auth.agentAddress);
  ...
  app.get("/", async (c) => {
    ...
    const tasks = statusFilter
      ? await db.listTasksForAgentByStatus(auth.agentAddress, statusFilter)
      : await db.listTasksForAgent(auth.agentAddress);
